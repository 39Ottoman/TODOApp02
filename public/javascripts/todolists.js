  ...
  $lists.fadeOut(function () {
    ...
    $.get('todolists', function (todoLists) {
      ...
      $.each(todoLists, function (index, todoList) {
        var listButton = '<div class="panel" id="' + todoList._id + '">' + '<div class="todoListName"><a href="/todopage/' + todoList._id + '">' + todoList.name + '</a></div>' + '<div class="info"></div>' + '</div>';
...
function showListInformation(listId) {
  ...
  $.get('/todos/' + listId, function (todos) {
    ...
    if (todos.length === 0) {
      ...
      message = '<div>' + todoCount + '個中' + checkCount + '個がチェック済み</div>' + '<div>～ ' + recentLimitDate.toLocaleDateString('ja-JP') + '</div>';
      console.log(message);
    }
    // ToDoリストの詳細として表示する
    $infoBox.html('');
